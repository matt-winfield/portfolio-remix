...
export const imageHandler = async (body: RaPayload) => {
    switch (body.method) {
        case 'create': {
            const transformedBody = await transformRequestBody(body);
            return await createHandler(transformedBody, prisma.image);
        }
        case 'update': {
            const transformedBody = await transformRequestBody(body);
            return await updateHandler(transformedBody, prisma.image);
        }
        default:
